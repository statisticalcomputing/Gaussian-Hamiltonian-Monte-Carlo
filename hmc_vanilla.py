''' stochastic hmc, delta and E_total from small to large, never decrease
'''
import numpy as np
...
if __name__ == '__main__':
    ...
    for delta in DELTA:
        ...
        for i in range(len(rho)):
            ...
            info = hmc(U, dU, D=D, EPISODE=EPISODE, WRONG=True)
            x = info['x']
            d0 = np.sum(np.square(np.cov(np.transpose(x[int(EPISODE/2):,:])) - SIGMA))
            ds.append(d0)
            info = hmc(U, dU, D=D, EPISODE=EPISODE, WRONG=False)
            x = info['x']
            d0 = np.sum(np.square(np.cov(np.transpose(x[int(EPISODE/2):,:])) - SIGMA))
            ds1.append(d0)
        DS.append(ds)
